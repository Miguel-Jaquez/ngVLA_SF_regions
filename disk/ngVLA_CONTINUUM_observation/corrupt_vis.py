...
a_cont,b_cont,c_cont = sigma_ps_fn("main",  freq=93, type_cal = 'line',delta_v = 1074514.1938 , theta=-1, t_int=tsource, verbose=True )

sigma_NA = a_cont[1]*1e-6   #in Jy/beam  #original in uJy/beam 
print ("continuum noise = {} Jy/beam".format(sigma_NA))
...
os.system('cp -r disk_93GHz_cont_{0}/disk_93GHz_cont_{0}.ngvla-revD.main.ms disk_93GHz_cont_{0}_noisy.ms'.format(line))

## Open the MS we want to add noise to with the sm tool:
sm.openfromms('disk_93GHz_cont_{}_noisy.ms'.format(line))

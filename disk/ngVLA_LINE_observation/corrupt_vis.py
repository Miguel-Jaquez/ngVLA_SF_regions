...
tsource = 3600*2
a_line,b_line,c_line = sigma_ps_fn("main",  freq=93,type_cal = 'line', theta=-1, t_int=tsource,delta_v=5.e3, verbose=True )
...
#os.system('cp -r disk_93GHz_line_{0}/disk_93GHz_line_{0}.ngvla-revD.main.ms disk_93GHz_line_{0}_noisy.ms'.format(line))

## Open the MS we want to add noise to with the sm tool:
#sm.openfromms('disk_93GHz_line_{}_noisy.ms'.format(line))

...
line = 'H41'
t_hr = 5 
tsource = 3600 * t_hr #seg
model = 'img_rl_jet_line_{}_700pc'.format(line)
simobserve(project = 'jet_93GHz_line_H{}_timeint_{}'.format(line, t_hr), 
                    skymodel = model+'.fits', 
                    setpointings = True,
                    ...
                    hourangle = 'transit', 
                    totaltime = '{}s'.format(tsource), #2hr  
                    thermalnoise = '',
                    overwrite=True,
                    graphics = 'none',
                    verbose = True)
